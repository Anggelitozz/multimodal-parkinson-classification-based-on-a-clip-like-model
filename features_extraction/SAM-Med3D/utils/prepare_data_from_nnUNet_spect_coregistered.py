...
def normalize_spect(input_path, output_path):
    img = tio.ScalarImage(input_path)
    data = img.data.numpy()[0]
    data_norm = min_max_norm(data)
    img.data = torch.from_numpy(data_norm).unsqueeze(0).float()
...
def resample_nii(input_path: str, output_path: str, target_spacing=(1.5, 1.5, 1.5), reference_image=None):
    ...
    # Obtener tensor (C, X, Y, Z)
    img = resampled_subject.img
    data = img.data.numpy()[0]   # (X,Y,Z)
...
# --- Rutas ---
base_path = Path("/home/Data/franklin_pupils/angel/PPMI")

base_path_atlas = Path("/home/Data/franklin_pupils/angel/PPMI")
output_base = Path("/home/Data/franklin_pupils/angel/SAM-Med3D-main/data/train_test/harvard_oxford_atlas/SPECT_full_pd_regions_smoothed")
...
for subject_id in tqdm(os.listdir(base_path_atlas), desc="Procesando sujetos"):
    subject_folder = base_path / subject_id / "preprocessed2"
    subject_folder_atlas = base_path_atlas / subject_id / "preprocessed" / "parcelations" / "full_pd_regions"
    # if not subject_folder.exists():
    #     print("no existe")
    #     continue

    # Buscar spect
    # mri_files = list(subject_folder.glob("PPMI_*_resampled_gaussian.nii.gz"))
    # if len(mri_files) == 0:
    #     print(f"No SPECT encontrado para {subject_id}")
    #     continue
    # mri_file = mri_files[0]

    # Buscar Label
    label_files = list(subject_folder_atlas.glob("Anon_atlas_labels_filtered.nii.gz"))
    if len(label_files) == 0:
        print(f"No Label encontrado para {subject_id}")
        continue
    label_file = label_files[0]

    spect_files = list(subject_folder.glob("*_resampled_gaussian.nii.gz"))
    if len(spect_files)==0:
        print(f"No SPECT encontrado para {subject_id}")
        continue
    spect_file = spect_files[0]
    # Rutas de salida
    mri_out = imagesTr_path / spect_file.name
    print(mri_out)
    label_out = labelsTr_path / spect_file.name  # mismo nombre que Spect

    # Copiar MRI
    normalize_spect(str(spect_file), str(mri_out))

    # Copiar Label (opcional: resample)
    reference_image = tio.ScalarImage(str(spect_file))
...
path = "/home/Data/franklin_pupils/angel/data/SPECT/harvard_oxford_atlas/imagesTr/5001_Anon_20201120160434_1_resampled_bias_corrected_rigid_registered.nii.gz"
