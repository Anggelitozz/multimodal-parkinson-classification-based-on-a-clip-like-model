...
def resample_nii(input_path: str, output_path: str, target_spacing=(1.5, 1.5, 1.5), reference_image=None):
    ...
    subject = tio.Subject(img=tio.LabelMap(input_path))  # usar LabelMap para máscaras
...
# --- Rutas ---
base_path = Path("/home/Data/franklin_pupils/angel/dataset_final")
output_base = Path("/home/Data/franklin_pupils/angel/SAM-Med3D-main/data/train_test/harvard_oxford_atlas/MRI_brain-stem")
...
for subject_id in tqdm(os.listdir(base_path), desc="Procesando sujetos"):
    ...
    label_out = labelsTr_path / mri_file.name

    # Copiar MRI
    if not mri_file.exists():
        print(f"MRI file no existe: {mri_file}")
    # print(mri_file)
    # print(mri_out)
