...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = attn @ v  # [B*num_heads, tokens, dim]

        # reshape explícito
        out_heads = out.view(
            B, self.num_heads, D, H, W, -1
        )

        if getattr(self, "return_head_embeddings", False):
            # global average pooling espacial
            head_embeds = out_heads.mean(dim=(2,3,4))  # [B, heads, dim_head]
            self.last_head_embeddings = head_embeds.detach()
